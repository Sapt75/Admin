...
const Add_New_Car = () => {
  ...
  async function manageBrand(itm) {
    let data = await fetch(`/modify_brand/${itm.brand}`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json"
      },
        ...
        <div className='mx-[4rem]'>
          <form onSubmit={handelData}>
            <div className='flex'>
              ...
              <input disabled onChange={changeInput} value={brandd.brand} required autocomplete="null" name='brand' className='px-[2rem] py-[0.5rem]' type="text" />
            </div>
